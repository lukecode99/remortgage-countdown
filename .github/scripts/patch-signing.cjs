...
const appProfile = process.env.RM_PROVISION_APP_NAME;
const widgetProfile = process.env.RM_PROVISION_WIDGET_NAME;
const team = process.env.IOS_TEAM_ID;
if (!pbxPath || !appProfile || !widgetProfile || !team) {
  console.error('usage: patch-signing.cjs <project.pbxproj> (env: RM_PROVISION_APP_NAME, RM_PROVISION_WIDGET_NAME, IOS_TEAM_ID)');
  process.exit(1);
}
